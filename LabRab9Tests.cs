...
namespace LabRab9.Tests
{
    ...
    public class ProgramTests
    {
        ...
        public void TestIncrementOperation()
        {
            ...
            // Assert
            Assert.AreEqual(10, money.Rubles, "Значение рублей должно остаться без изменений после инкремента");
            Assert.AreEqual(51, money.Kopeks, "Значение копеек изменится на 1 после инкремента");
        }
        ...
        public void TestDecrementOperation()
        {
            ...
            Assert.AreEqual(99, money.Kopeks, "Копейки должны быть 100 после декремента для корректного значения");
        }
        ...
        public void TestImplicitConversionToDouble()
        {
            ...
            // Assert
            Assert.AreEqual(0.75, kopeks, "Приведение к double должно возвращать полное значение в копейках");
        }
    ...
    public class MoneyArrayTests
    {
        ...
        public void Constructor_Default_SetsArrayToSize10WithDefaultMoney()
        {
            ...
            {
                var money = moneyArray[i]; // Assuming we have an indexer in MoneyArray
                Assert.IsNotNull(money, "Each Money element in the array should be initialized.");
                Assert.AreEqual(0, money.Rubles, "Default rubles should be 0.");
                Assert.AreEqual(0, money.Kopeks, "Default kopeks should be 0.");
            }
        }
        ...
        public void Constructor_WithSizeAndRandom_SetsArrayToSizeWithRandomMoney()
        {
            ...
            // Assert
            Assert.AreEqual(size, moneyArray.Length, "The length of the array should match the specified value.");
            for (int i = 0; i < moneyArray.Length; i++)
            {
                var money = moneyArray[i];
                Assert.IsNotNull(money, "Each Money element in the array should be initialized.");
                Assert.IsTrue(money.Rubles >= 0 && money.Rubles < 100, "Rubles should be in the range 0-99.");
                Assert.IsTrue(money.Kopeks >= 0 && money.Kopeks < 100, "Kopeks should be in the range 0-99.");
            }
        }
        ...
        public void Display_PrintsAllElementsToConsole()
        {
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                Assert.IsTrue(actualOutput.Length < 1000, "Вывод слишком велик!");

                Assert.AreEqual(expectedOutput, actualOutput, "Display должен корректно выводить элементы массива.");
            }
        }
